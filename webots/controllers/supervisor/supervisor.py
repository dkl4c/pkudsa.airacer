...
for cc in cars_config:
    node = robot.getFromDef(cc['car_node_id'])
    cars.append({
        "team_id":             cc['team_id'],
        "car_node_id":         cc['car_node_id'],
...
while robot.step(timestep) != -1:
    ...
    for car in cars:
        ...
        car['x'], car['y'] = pos[0], pos[2]          # x=right, z=depth → 2D plane

        ori = car['node'].getOrientation()            # row-major 3x3 rotation matrix
        car['heading'] = math.atan2(-ori[2], ori[0])

        vel = car['node'].getVelocity()               # [vx, vy, vz, wx, wy, wz]
        car['speed'] = math.sqrt(vel[0] ** 2 + vel[2] ** 2)
